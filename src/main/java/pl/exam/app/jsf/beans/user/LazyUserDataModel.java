...
public class LazyUserDataModel extends LazyDataModel<User> {
    ...
    public User getRowData(String rowKey) {
        return userRepository.findById(Integer.valueOf(rowKey)).orElseThrow();
    }
    ...
    private Sort createSortSpec(String sortField, SortOrder sortOrder) {
        if (sortField == null) {
            return Sort.by(Sort.Order.asc("schoolClass"), Sort.Order.asc("idInClass")
            , Sort.Order.desc("creationDate"));
        }
        return Sort.by(toSortDirction(sortOrder), sortField);
    }
